...
/// </summary>
internal class EventStore(TableClient tableClient, IGrainStorageSerializer serializer, IOptions<ClusterOptions> clusterOptions)
{
    ...
    private string CreateRowKey(GrainId grainId)
    => tableClient.SanitizeKeyPropertyValue(
        key: $"{clusterOptions.Value.ClusterId}{StreamSeparator}{grainId.ToString()}",
        sanitizeChar: StreamSeparator);
