...
public abstract partial class EventSourcedGrain<TEvent, TState> : Grain
{
    ...
    public ValueTask<int> RaiseEventAsync(TEvent @event, CancellationToken cancellationToken = default)
        => RaiseEventsAsync([@event], cancellationToken);

    /// <summary>
    /// Saves the <paramref name="events"/> to the <typeparamref name="TEvent"/> log and applies them to the <see cref="State"/>.
    /// </summary>
    /// <remarks>
    /// This does NOT save the projected <see cref="State"/> to storage. Explicitly
    /// call <see cref="WriteStateAsync"/> to save the state.
    /// </remarks>
    /// <returns>The number of the <paramref name="events"/> that were successfully saved and applied.</returns>
    public async ValueTask<int> RaiseEventsAsync(IReadOnlyCollection<TEvent> events, CancellationToken cancellationToken = default)
    {
        var appendedEvents = await eventLogStorage.AppendEventsAsync(events, CancellationToken.None);

        if (appendedEvents != events.Count)
        {
            LogFailedAppendAllEvents(appendedEvents, events.Count);
        }
        ...
        {
            if (appendedEvents == appliedEvents)
            {
                break;
            }
        ...
        if (appendedEvents != appliedEvents)
        {
            LogFailedToApplyAllAppendedEvents(appliedEvents, appendedEvents);
        }
    ...
    private async ValueTask ReadStateAsync(bool writeStateIfStoredStateInvalid, CancellationToken cancellationToken)
    {
        ...
        if (writeStateIfStoredStateInvalid)
        {
    ...
    protected virtual TState CreateDefaultState()
        => ActivatorUtilities.CreateInstance<TState>(serviceProvider);
