...
>
> Run benchmarks locally with `dotnet run --project perf/Egil.SystemTextJson.Migration.PerfTests -c Release --framework net11.0`.
...
- **Non-object payload migration.** When the JSON payload is not an object (e.g., an array or primitive), discriminator-based matching is not possible. The library matches migrators by comparing the JSON token type against the source type's `JsonTypeInfoKind` (`StartArray` → `Enumerable`, primitives → `None`). A JSON number matches any numeric source type STJ can read, including `Half`, `Int128`, and the .NET 11 `BFloat16`/`Decimal32`/`Decimal64`/`Decimal128` types; `Nullable<T>` sources are matched by their underlying type. Dictionary source types (`Dictionary<string, T>`) are also supported — when no discriminator match is found on a JSON object, the library checks for `JsonTypeInfoKind.Dictionary` migrators before falling back to legacy handling. This adds zero overhead to the existing object-based happy path.
