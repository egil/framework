...
6. **Recursive models** such as `Branch(Node[] Children)` with `union Node(Branch, Leaf)` are supported. The converter of a migratable type resolves that type's own object contract without migration (its recursion guard), so a nested old-version payload of the *same* type as an enclosing one (an old `Branch` inside a `Branch`) is read as the current version rather than migrated; nested old payloads of other types migrate as usual.
