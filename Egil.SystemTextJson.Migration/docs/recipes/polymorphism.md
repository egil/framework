...
3. **Arrays, strings, numbers and booleans** go to the single case with that JSON shape. Numbers use the same rules as [non-object payload migration](migration-authoring.md#migrating-from-non-object-json-payloads); string-shaped cases are `string`, `char`, `DateTime`, `DateTimeOffset`, `DateOnly`, `TimeOnly`, `TimeSpan`, `Guid`, `Uri`, `Version` and `byte[]` (converter overrides such as `JsonStringEnumConverter` are not detected). If any case is a nested union or uses a custom converter, it may accept any shape, so no shape-based fallback is attempted and only discriminated payloads are classified.
4. **`null`** never reaches the classifier: STJ yields the nullable case or the default union value.
