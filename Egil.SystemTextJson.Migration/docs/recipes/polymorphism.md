...
3. **Arrays, strings, numbers and booleans** go to the single case with that JSON shape (numbers use the same rules as [non-object payload migration](migration-authoring.md#migrating-from-non-object-json-payloads)). If any case is a nested union or uses a custom converter, it may accept any shape, so no shape-based fallback is attempted and only discriminated payloads are classified.
