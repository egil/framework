...
6. **Recursive models** such as `Branch(Node[] Children)` with `union Node(Branch, Leaf)` are supported; a document is written by one version of the model, so migrating the root migrates the tree.
