...
2. **Guard:** if any case is a nested union, uses a converter override (`[JsonConverter]` on the type or a matching entry in `options.Converters`, such as `JsonStringEnumConverter`) or has a source type with such a converter, that case may accept any JSON shape. The union then classifies discriminated payloads only (a nested union contributes the discriminators of its own migratable cases, recursively, and an overridden object-like source contributes its discriminator); every payload without a leading discriminator throws.
