...
2. **Guard:** if any case is a nested union, uses a converter override (`[JsonConverter]` on the type or a matching entry in `options.Converters`, such as `JsonStringEnumConverter`) or has a source type with such a converter, that case may accept any JSON shape. The union then classifies discriminated payloads only (a nested union contributes the discriminators of its own migratable cases and of its overridden object-like cases and sources, recursively); every payload without a leading discriminator throws.
...
5. **`null`** never reaches the classifier: STJ yields the nullable case or the default union value. A nullable migratable struct case (`ValueLeaf?`) otherwise routes exactly like `ValueLeaf`.
6. **Recursive models** such as `Branch(Node[] Children)` with `union Node(Branch, Leaf)` are supported. Model the old version with old types all the way down (`OldBranch(OldNode[] Children)` with `union OldNode(OldBranch, Leaf)`) and let `TryMigrateFrom` walk the children; every level then migrates. A source type that references the *current* union for its children instead cannot migrate a nested old node of the target itself (inside the target's own migration the target resolves to its plain contract), so such a payload throws a `JsonException` naming the discriminator rather than being read as the current version.
