...
> **Note:** Non-object payloads have no type discriminator. Collection sources are matched by contract kind (`StartArray` → `JsonTypeInfoKind.Enumerable`; a non-empty JSON object with no recognized discriminator → `JsonTypeInfoKind.Dictionary`; an empty object `{}` is deserialized as the target type). A target that configures `UndiscriminatedSourceType` takes precedence: every object without a recognized discriminator, empty or not, is migrated from that source before dictionary migrators are considered. Scalar sources are matched by the token family their built-in converter reads, not by contract kind: a `String` token matches `string`, `char`, `DateTime`, `DateTimeOffset`, `DateOnly`, `TimeOnly`, `TimeSpan`, `Guid`, `Uri`, `Version`, `byte[]`, `Memory<byte>` and `ReadOnlyMemory<byte>` (base64); `True`/`False` match `bool`; a `Number` token matches the numeric types with built-in STJ converters (`int`, `long`, `double`, `decimal` and the other `TypeCode` numerics, enums, `Half`, `Int128`, `UInt128`, and on .NET 11 `BFloat16`/`Decimal32`/`Decimal64`/`Decimal128`); types such as `BigInteger` that need a custom converter are not numeric sources, and a source with a converter override (`[JsonConverter]` on the type, a matching entry in `options.Converters` such as `JsonStringEnumConverter`, or a custom converter type attached by the type info resolver) is not shape-matched alongside the others because the override may read a different token family; it is matched by the token family its CLR type would read (an `int` takes a number, an enum a number, a `string` a string), which is how 1.x matched every source, in the tier described below: an overridden source of a type 1.x knew comes right after the plain ones of those types, and an overridden source of a type 2.0 added comes last. A resolver that attaches a differently configured instance of a built-in converter type is not detected. A collection source whose element type has such an override is selected only when it is the sole collection candidate; alongside other collection sources the payload is reported as ambiguous. With `JsonNumberHandling.AllowReadingFromString` (on by default with `JsonSerializerDefaults.Web`) a quoted number also reaches a numeric source (enums excluded, their converter reads strings only as names) when no `string` source is registered; `"NaN"`/`"Infinity"`/`"-Infinity"` reach floating-point sources only, under either that flag or `AllowNamedFloatingPointLiterals`. Scalar sources are tried in tiers: the types 1.x matched by `TypeCode` (`string`, `bool`, the primitive numerics and enums) first, plain ones before ones behind a converter override, then the types 2.0 added, then quoted numbers (again 1.x's numeric types before 2.0's), then overridden sources of the 2.0 types; two candidates in the same tier are ambiguous and throw, so `int` and `long` sources on one target cannot share a number payload while `int` and `Half` can, with `int` winning as it did in 1.x. Object payloads continue to use discriminator-based matching. After migration, the target type is serialized as an object with `$type`, so future reads take the zero-allocation happy path.
