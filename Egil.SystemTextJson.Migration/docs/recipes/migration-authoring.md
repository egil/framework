...
> **Note:** Non-object payloads have no type discriminator. The library matches migrators by comparing the JSON token type (`StartArray`, `String`, `Number`, `True`/`False`) against the source type's `JsonTypeInfoKind`. A `Number` token matches any numeric source type STJ can read from a number (`int`, `long`, `double`, `decimal`, `Half`, `Int128`, and on .NET 11 `BFloat16`/`Decimal32`/`Decimal64`/`Decimal128`); registering two numeric sources for the same target is ambiguous and throws. Object payloads continue to use discriminator-based matching. After migration, the target type is serialized as an object with `$type`, so future reads take the zero-allocation happy path.
