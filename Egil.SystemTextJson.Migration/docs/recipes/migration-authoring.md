...
> **Note:** Non-object payloads have no type discriminator. Collection sources are matched by contract kind (`StartArray` → `JsonTypeInfoKind.Enumerable`; a JSON object with no recognized discriminator → `JsonTypeInfoKind.Dictionary`). Scalar sources are matched by the token family their built-in converter reads, not by contract kind: a `String` token matches `string`, `char`, `DateTime`, `DateTimeOffset`, `DateOnly`, `TimeOnly`, `TimeSpan`, `Guid`, `Uri`, `Version` and `byte[]` (base64); `True`/`False` match `bool`; a `Number` token matches the numeric types with built-in STJ converters (`int`, `long`, `double`, `decimal` and the other `TypeCode` numerics, enums, `Half`, `Int128`, `UInt128`, and on .NET 11 `BFloat16`/`Decimal32`/`Decimal64`/`Decimal128`); types such as `BigInteger` that need a custom converter are not numeric sources, and a source with a converter override (`[JsonConverter]` on the type, a matching entry in `options.Converters` such as `JsonStringEnumConverter`, or a custom converter type attached by the type info resolver) is never shape-matched because the override may read a different token family. A resolver that attaches a differently configured instance of a built-in converter type is not detected. A collection source whose element type has such an override is selected only when it is the sole collection candidate; alongside other collection sources the payload is reported as ambiguous. With `JsonNumberHandling.AllowReadingFromString` (on by default with `JsonSerializerDefaults.Web`) a quoted number also reaches a numeric source when no `string` source is registered; `AllowNamedFloatingPointLiterals` does the same for `"NaN"`/`"Infinity"` on floating-point sources only. Registering two numeric sources for the same target is ambiguous and throws. Object payloads continue to use discriminator-based matching. After migration, the target type is serialized as an object with `$type`, so future reads take the zero-allocation happy path.
