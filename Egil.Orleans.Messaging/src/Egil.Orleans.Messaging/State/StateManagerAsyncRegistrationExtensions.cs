...
public static partial class StateManagerExtensions
{
    extension<TGrain>(TGrain grain) where TGrain : IGrainBase
    {
        ...
        public Task<IStateManager<TState>> RegisterStateManagerAsync<TState>(
            IPersistentState<TState> storage, StateManagerHooks<TState> hooks,
            Action<TState>? configureState = null, CancellationToken cancellationToken = default)
            where TState : class, IEquatable<TState>
            => RegisterInitializedManagerAsync(grain, null, storage, null, configureState, hooks, cancellationToken);

        /// <summary>Registers a hydrated manager with a keyed factory and awaits its initial read handlers.</summary>
        public Task<IStateManager<TState>> RegisterStateManagerAsync<TState>(
            string storageName, IPersistentState<TState> storage, StateManagerHooks<TState> hooks,
            Action<TState>? configureState = null, CancellationToken cancellationToken = default)
            where TState : class, IEquatable<TState>
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(storageName);
            return RegisterInitializedManagerAsync(grain, storageName, storage, null, configureState, hooks, cancellationToken);
        }

        /// <summary>Registers a hydrated manager with an explicit default and awaits its initial read handlers.</summary>
        public Task<IStateManager<TState>> RegisterStateManagerAsync<TState>(
            IPersistentState<TState> storage, Func<TState> createInitialState, StateManagerHooks<TState> hooks,
            Action<TState>? configureState = null, CancellationToken cancellationToken = default)
            where TState : class, IEquatable<TState>
        {
            ArgumentNullException.ThrowIfNull(createInitialState);
            return RegisterInitializedManagerAsync(grain, null, storage, createInitialState, configureState, hooks, cancellationToken);
        }
        ...
        public Task<IStateManager<TState>> RegisterStateManagerAsync<TState>(
            ...
            StateManagerHooks<TState> hooks, Action<TState>? configureState = null,
            CancellationToken cancellationToken = default)
            where TState : class, IEquatable<TState>
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(storageName);
            ArgumentNullException.ThrowIfNull(createInitialState);
            return RegisterInitializedManagerAsync(grain, storageName, storage, createInitialState, configureState, hooks, cancellationToken);
        }
    }
    ...
    private static async Task<IStateManager<TState>> RegisterInitializedManagerAsync<TState>(
        ...
        Action<TState>? configureState, StateManagerHooks<TState> hooks, CancellationToken cancellationToken)
        where TState : class, IEquatable<TState>
    {
