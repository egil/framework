...
[CmdletBinding()]
param(
    [string]$TagPrefix = "egil-orleans-messaging/v",
    [string]$ProjectPath = "Egil.Orleans.Messaging/"
)
...
# Anchor the path at the repository root so callers can invoke the script from any subdirectory.
$projectPathspec = ":(top)$ProjectPath"
$commitArgs = @('log', $range, "--format=${commitSeparator}%n%s%n%b", '--no-merges', '--', $projectPathspec)
